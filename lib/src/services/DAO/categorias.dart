...
Future listaPeticiones(cuerpoCategoria, listaAnimales) async {
  var nombre = await addCategoria(
      'http://192.168.0.2:3000/categoria/createCategoria', cuerpoCategoria);
  Future.delayed(Duration(milliseconds: 2000), () {
    findNameCategory("http://192.168.0.2:3000/categoria/findCategoryByName",
  ...
  if (response.statusCode == 200) {
    ...
    if (listaVaca.length > 0) {
      ...
      var x = await createAnimalCategory(
          "http://192.168.0.2:3000/categoria/createCategoryVaca",
          listaVaca.toString());
    }
    if (listaToro.length > 0) {
      print(listaToro);
      var x = await createAnimalCategory(
          "http://192.168.0.2:3000/categoria/createCategoryToro",
          listaToro.toString());
    }
    if (listaBecerro.length > 0) {
      print(listaBecerro);
      var x = await createAnimalCategory(
          "http://192.168.0.2:3000/categoria/createCategoryBecerro",
