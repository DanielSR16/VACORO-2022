                  ...
                  child: SearchableList<Becerros>(
                    ...
                    onItemSelected: (Becerros item) async {
                      ...
                      Widget continueButton = ElevatedButton(
                        child: Text("Eliminar"),
                        onPressed:  () async {
                          var cuerpoEliminar = jsonEncode({"id_categoria":widget.categoriaSeleccionada.idCategoria,"id_becerro":item.id,"id_usuario":2});
                          var x = await deleteAnimalCategoryById("http://192.168.100.6:3006/categoria/deleteBecerroByCategory", cuerpoEliminar);
          ...
          FloatingActionButton(
            onPressed: () async {
              if (aceptacion.length > 0) {
                ...
                var agrego = await updateBecerroByCategory("http://192.168.100.6:3006/categoria/updateBecerroByCategory",cuerpoAgregar);
                var id = jsonEncode({"id_becerro":idOpcion});
                var infoAnimal = await infoAnimalById("http://192.168.100.6:3006/categoria/findByIdBecerro", id);
