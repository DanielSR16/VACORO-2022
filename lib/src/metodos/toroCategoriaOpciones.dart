                  ...
                  child: SearchableList<Toros>(
                    ...
                    onItemSelected: (Toros item) async {
                      ...
                      Widget continueButton = ElevatedButton(
                        child: Text("Eliminar"),
                        onPressed:  () async {
                          var cuerpoEliminar = jsonEncode({"id_categoria":widget.categoriaSeleccionada.idCategoria,"id_toro":item.id,"id_usuario":2});
                          var x = await deleteAnimalCategoryById("http://192.168.100.6:3006/categoria/deleteToroByCategory", cuerpoEliminar);
          ...
          FloatingActionButton(
            onPressed: () async {
              if (aceptacion.length > 0) {
                ...
                var agrego = await updateBecerroByCategory("http://192.168.100.6:3006/categoria/updateToroByCategory",cuerpoAgregar);
                var id = jsonEncode({"id_toro":idOpcion});
                var infoAnimal = await infoAnimalById("http://192.168.100.6:3006/categoria/findByIdToro", id); //aca
                print(infoAnimal);
